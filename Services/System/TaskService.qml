...
Singleton {
        ...
        case "tag":
            {
                ...
                for (var j = 0; j < tasks.length; j++) {
                    var task2 = tasks[j];
                    if (task2 && task2.tags && task2.tags.indexOf(currentTag) !== -1)
                        byTag.push(task2);
                }
                return byTag;
            }
    ...
    Timer {
        ...
        interval: 150
        repeat: false
        onTriggered: root._startLoadTasks()
    }
    ...
    function _dequeueModify() {
        if (taskModifier.running)
            return;
        if (!_modifyQueue || _modifyQueue.length === 0)
            return;

        var item = _modifyQueue.shift();
        taskModifier.taskId = item.taskId;
        taskModifier.callback = item.callback;
        taskModifier.command = item.command;
    ...
    // Helper to check if date is today (local).
    function _isToday(dateStr) {
        return TaskwarriorDate.isToday(dateStr);
    }

    // Helper to check if date is in the future (local day).
    function _isFuture(dateStr) {
        return TaskwarriorDate.isFutureDay(dateStr);
    }
    ...
    function _filterTasks() {
        ...
        Logger.d("TaskService", "Filtered tasks - Inbox:", inbox.length, "Today:", today.length, "Upcoming:", upcoming.length, "Anytime:", anytime.length, "Someday:", someday.length);
    }
    ...
    JsonProcess {
        ...
        onJsonReady: function (data) {
            ...
            if (root._reloadQueued) {
                root.loadTasks(false);
            }
        }

        onJsonFailed: function (exitCode, reason) {
            root.loading = false;
            root.lastError = "Failed to load tasks";
            Logger.e("TaskService", root.lastError, "reason:", reason, "exitCode:", exitCode);
            root.tasks = [];
            root.pendingCount = 0;
            root.tasksUpdated();
        }
    ...
    JsonProcess {
        ...
        onJsonReady: function (data) {
            root.completedTasks = data || [];
            Logger.d("TaskService", "Loaded", root.completedTasks.length, "completed tasks");
        }

        onJsonFailed: function (exitCode, reason) {
            Logger.e("TaskService", "Failed to load completed tasks", "reason:", reason, "exitCode:", exitCode);
            root.completedTasks = [];
        }
    }
