...
ColumnLayout {
  ...
  ColumnLayout {
    ...
    NToggle {
      ...
      description: ((Settings.data.bar.mode ?? "classic") === "framed" && !Settings.data.general.showScreenCorners)
                   ? "⚠ Recommended to enable with Framed bar mode"
  ...
  // Screen Border has moved to Bar tab → Bar Mode → Framed
  // This keeps the UI cleaner with all bar-related settings in one place
